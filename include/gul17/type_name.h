...
/**
 * \addtogroup type_name_h gul17/type_name.h
 ...
 * gul17::FailToInstantiate<decltype(t)>{};
 * \endcode
...
constexpr std::string_view type_name()
{
    ...
    auto s = std::string_view{ static_cast<const char*>(__PRETTY_FUNCTION__) };
    auto const start_idx = s.find(" = ") + 3; // len(" = ") == 3
    s.remove_prefix(start_idx);

    auto const colon_idx = s.find(";");
    auto const suff_length =
        (colon_idx != std::string_view::npos) ? s.length() - colon_idx : 1; // len("]") == 1
    s.remove_suffix(suff_length);
    return s;
#elif defined(_MSC_VER)
    // MSVC returns something like "return_type function_name<template_parameter>()"
    auto s = std::string_view{ __FUNCSIG__ };
    auto const start_idx = s.find("gul17::type_name<") + sizeof("gul17::type_name<") - 1;
    s.remove_prefix(start_idx);
