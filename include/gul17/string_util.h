...
#ifndef GUL17_STRING_UTIL_H_
#define GUL17_STRING_UTIL_H_
...
/**
 * \addtogroup string_util_h gul17/string_util.h
 ...
 */
GUL_EXPORT
extern const std::string_view default_whitespace_characters;
...
template <typename Integer, size_t num_elements,
          std::enable_if_t<std::is_integral<Integer>::value, bool> = true>
inline std::string
hex_string(const Integer (&array)[num_elements], std::string_view separator = "")
{
 ...
 *                   visually (empty by default)
 *
 * \tparam Container  must be a gul17::IsContainerLike class
 *
 * \since GUL version 2.6
 */
...
inline std::string
hex_string(const Container& container, std::string_view separator = "")
{
