...
/**
 * \addtogroup substring_checks_h gul17/substring_checks.h
 ...
 *          otherwise.
 *
 * \see contains(std::string_view, char), contains_nocase(std::string_view, std::string_view),
 *      ends_with(std::string_view, std::string_view), starts_with(std::string_view, std::string_view)
 */
constexpr inline bool contains(std::string_view haystack, std::string_view needle) noexcept
{
 ...
 *          otherwise.
 *
 * \see contains(std::string_view, std::string_view), contains_nocase(std::string_view, char),
 *      ends_with(std::string_view, char), starts_with(std::string_view, char)
 */
constexpr inline bool contains(std::string_view haystack, char needle) noexcept
{
    return haystack.find(needle) != std::string_view::npos;
}
 ...
 *
 * \see ends_with(std::string_view, char), ends_with_nocase(std::string_view, std::string_view),
 *      starts_with(std::string_view, std::string_view), contains(std::string_view, std::string_view)
 */
 ...
 *
 * \see ends_with(std::string_view, std::string_view), ends_with_nocase(std::string_view, char),
 *      starts_with(std::string_view, char), contains(std::string_view, char)
 */
constexpr inline bool ends_with(std::string_view str, char c) noexcept
{
 ...
 *
 * \see starts_with(std::string_view, char), starts_with_nocase(std::string_view, std::string_view),
 *      ends_with(std::string_view, std::string_view), contains(std::string_view, std::string_view)
 */
constexpr inline bool starts_with(std::string_view str, std::string_view prefix) noexcept
{
    ...
    return hsl >= hl && std::string_view{ str.data(), hl }.compare(prefix) == 0;
}
 ...
 *
 * \see starts_with(std::string_view, std::string_view), starts_with_nocase(std::string_view, char),
 *      ends_with(std::string_view, char), contains(std::string_view, char)
 */
constexpr inline bool starts_with(std::string_view str, char c) noexcept
{
    return !str.empty() && str.front() == c;
}

//
// Case insensitive std::variants following
//
 ...
 *
 * \see contains_nocase(std::string_view, std::string_view),
 *      ends_with_nocase(std::string_view, std::string_view),
 *      starts_with_nocase(std::string_view, std::string_view)
 */
constexpr inline bool equals_nocase(std::string_view str1, std::string_view str2) noexcept
{
 ...
 *
 * \see contains_nocase(std::string_view, char), contains(std::string_view, std::string_view),
 *      ends_with_nocase(std::string_view, std::string_view),
 *      starts_with_nocase(std::string_view, std::string_view)
 */
constexpr inline bool contains_nocase(std::string_view haystack, std::string_view needle) noexcept
{
 ...
 * \since GUL version 1.7
 *
 * \see contains_nocase(std::string_view, std::string_view), contains(std::string_view, char),
 *      ends_with_nocase(std::string_view, char), starts_with_nocase(std::string_view, char)
 */
constexpr inline bool contains_nocase(std::string_view haystack, char needle) noexcept
{
 ...
 * \since GUL version 1.7
 *
 * \see ends_with_nocase(std::string_view, char), ends_with(std::string_view, std::string_view),
 *      starts_with_nocase(std::string_view, std::string_view),
 *      contains_nocase(std::string_view, std::string_view)
 */
constexpr inline bool ends_with_nocase(std::string_view str, std::string_view suffix) noexcept
{
    while (not str.empty()) {
        if (suffix.empty())
            return true;
 ...
 * \since GUL version 1.7
 *
 * \see ends_with_nocase(std::string_view, std::string_view), ends_with(std::string_view, char),
 *      starts_with_nocase(std::string_view, char), contains_nocase(std::string_view, char)
 */
constexpr inline bool ends_with_nocase(std::string_view str, char c) noexcept
{
 ...
 * \since GUL version 1.7
 *
 * \see starts_with_nocase(std::string_view, char), starts_with(std::string_view, std::string_view),
 *      ends_with_nocase(std::string_view, std::string_view),
 *      contains_nocase(std::string_view, std::string_view)
 */
constexpr inline bool starts_with_nocase(std::string_view str, std::string_view prefix) noexcept
{
    while (not str.empty()) {
        if (prefix.empty())
            return true;
 ...
 * \since GUL version 1.7
 *
 * \see starts_with_nocase(std::string_view, std::string_view), starts_with(std::string_view, char),
 *      ends_with_nocase(std::string_view, char), contains_nocase(std::string_view, char)
 */
constexpr inline bool starts_with_nocase(std::string_view str, char c) noexcept
{
