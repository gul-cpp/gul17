...
/**
 * \addtogroup to_number_h gul17/to_number.h
...
/**
 * Convert two std::string_views into one floating point number.
 ...
 *
 * \returns a ParseInfNanResult that contains a std::optional with the number
 */
template <typename NumberType>
...
/**
 * Convert a std::string_view into a floating point number.
 ...
 *          the returned std::optional is empty. If nothing special has been found
 ...
 * \param str  The signless string to be converted.
 *
 * \returns a std::optional that contains the number if the conversion was successful. If
...
/**
 * Convert a std::string_view into a floating point number using std::strtold.
 ...
 *
 * \returns an std::optional that contains std::strtold()'s return value (include HUGE_VAL).
 ...
 * This function parses the ASCII representation of a number (e.g. "123" or "1.3e10") into
 * an std::optional integer or floating-point number.
 *
 * \code
 * std::optional<int> result = gul17::to_number<int>("42");
 *
 * if (result.has_value())
 ...
 * - Its input type is std::string_view: No null-termination and no temporary std::string are
 ...
 * * Not-a-number expressions: (std::optional minus sign) NAN or NAN(char_sequence) ignoring case.
...
constexpr inline std::optional<NumberType> to_number(std::string_view str) noexcept
{
