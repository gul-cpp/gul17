...
#ifndef GUL17_NUM_UTIL_H_
#define GUL17_NUM_UTIL_H_
...
/**
 * \addtogroup num_util_h gul17/num_util.h
...
bool within_orders(const NumT a, const NumT b, const OrderT orders) noexcept(false) {
    ...
    auto difference = gul17::abs(a - b);
    if (difference == NumT{ 0 })
        return true;
    auto maximum = std::max(gul17::abs(a), gul17::abs(b));
...
bool within_abs(NumT a, NumT b, NumT tol) noexcept {
    tol = gul17::abs(tol); // Negative diff does not make sense
    bool ret{};
    if (a > b) {
